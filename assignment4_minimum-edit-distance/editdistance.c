	...
	for (int i = 1; i <= n; ++i) {
		for (int j = 1; j <= m; ++j) {
			...
			if ((i > 1 && j > 1) && (str1[i - 2] == str2[j-1] && str1[i-1] == str2[j - 2])) {
				...
				if (dp[i][j] == dp[i - 1][j] + DELETE_COST) op_matrix[i*(m+1)+j] += DELETE_OP;
				if (dp[i][j] == dp[i][j - 1] + INSERT_COST) op_matrix[i * (m + 1) + j] += INSERT_OP;
				if (dp[i][j] == dp[i - 1][j - 1] + cost) op_matrix[i * (m + 1) + j] += cost ? SUBSTITUTE_OP : MATCH_OP;
				if (dp[i][j] == dp[i - 2][j - 2] + TRANSPOSE_COST) op_matrix[i * (m + 1) + j] += TRANSPOSE_OP;
			} 
			else { 
