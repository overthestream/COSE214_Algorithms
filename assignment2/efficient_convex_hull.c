...
t_line* upper_hull(t_point* points, int num_point, t_point p1, t_point pn, t_line* lines, int* num_line, int* capacity) {
	if (!num_point) {
		lines = (t_line*)realloc(lines, (*num_line) + 1);
	...
	t_point p_max, to = pn, from = p1;
	double distM = 0;
	double a = to.y - from.y, b = from.x - to.x, c = from.x * to.y - from.y * to.x;

	for (int i = 0; i < num_point; i++) {
		if (distance(a, b, c, points[i) > distM) {
			distM = distance(a, b, c, points[i);
			p_max = points[i];
		}
	}
	...
	separate_points(points, num_point, p1, p_max, s1, s2, n1, n2);
	separate_points(points, num_point, p_max, pn, s2, s3, n2, n3);
...
void separate_points(t_point* points, int num_point, t_point from, t_point to, t_point* s1, t_point* s2, int* n1, int* n2) {
	...
	*n2 = 0;
	for (int i = 0; i < num_point; i++) {
		...
		if (a * curX + b * curY - c < 0) {
			s1 = (t_point*)realloc(s1, sizeof(t_point) * ((*n1) + 1);
			s1[(*n1)].x = curX;
			s1[(*n1)++].y = curY;

		}
		else if (a * curX + b * curY - c > 0) {
			s2 = (t_point*)realloc(s1, sizeof(t_point) * ((*n2) + 1);
			s2[(*n1)].x = curX;
			s2[(*n1)++].y = curY;
		}
	}
